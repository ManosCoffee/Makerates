...
WITH top_currencies AS (
    ...
    FROM {{ ref('fact_rates_validated') }}
    WHERE target_currency IN ('USD', 'EUR', 'GBP', 'JPY', 'AUD', 'CAD', 'CHF', 'CNY', 'HKD', 'SGD')
    AND rate_date = (SELECT MAX(rate_date) FROM {{ ref('fact_rates_validated') }})
),
...
latest_rates AS (
    ...
    FROM {{ ref('fact_rates_validated') }}
    WHERE rate_date = (SELECT MAX(rate_date) FROM {{ ref('fact_rates_validated') }})
)
